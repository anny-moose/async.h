 ...
 * @param ctx Pointer to the queue to initialize. Expected to not be previously
 * initialized.
 * @param num_threads The number of threads to be started. If for whatever
 * reason they can't be started, the queue is not initialized.
 * @returns 0 on success, -1 if any step fails
 *
 * ERRORS:
 * EINVAL - Invalid parameters (NULL ctx or num_threads < 1)
 * See man pages pthread_cond_init(3), pthread_mutex_init(3), malloc(3),
 * pthread_create(3) for other possible errors.
 */
int init_tq(thread_queue_t* ctx, int num_threads);
 ...
 *
 * All the promises that belong to this queue and haven't been previously
 * completed/started are cancelled. A cancelled promise will set errno to
 * ECANCELED, and the data pointer will be set to NULL. You must await the
 * canceled promises if you don't want to leak memory
 */
 ...
 * @param ctx Pointer to an initialized thread queue
 * @param callback The function that the thread will execute. Any function you
 * would call pthread_create with would work.
 ...
 */
promise_t* submit_task(thread_queue_t* ctx, void* (*callback)(void*),
                       void* arg);

/**
 * Submits a detached task to the thread queue. A detached task executes, and
 * it's return value is then discarded. See the documentation for submit_task
 * for more info.
 * @returns 0 on success, -1 on failure.
 */
int submit_task_detached(thread_queue_t* ctx, void* (*callback)(void*),
                         void* arg);
 ...
 * no longer be used after await, as it is de-allocated.
 */
 ...
 * EAGAIN - Promise could not be consumed immediatelly (in the case that timeout
 * is NULL).
 */
 ...
 * Should return 0 if not, any non-zero value if yes.
 ...
 * This function will not cancel any tasks that are ongoing, and those promises
...
void* __worker_func(void* arg) {
    ...
    while (1) {
        while (state->queue.length > 0) {
            task_t* task = (task_t*)queue_pop_head(&state->queue);
            ...
            if (task) {
                ...
                /* maybe should restore sigmask in case it gets changed by the
                 * task */
            }
...
int init_tq(thread_queue_t* ctx, int num_threads) {
    ...
    queue_init(&ctx->queue, sizeof(task_t), __free_func);
    ...
    sigfillset(&fullset);
    pthread_sigmask(SIG_SETMASK, &fullset,
                    &oldset); /* block all signals, as handling them in the
                                 worker threads would be troublesome */

    int initialized_threads;
    for (initialized_threads = 0; initialized_threads < num_threads;) {
        if (!pthread_create(&ctx->threads[initialized_threads], NULL,
                            __worker_func, ctx))
            initialized_threads++;
        else
...
fail_post_threads:
    pthread_sigmask(SIG_SETMASK, &oldset, NULL);
    for (int i = 0; i < initialized_threads; i++)
...
static inline promise_t* __init_promise() {
...
/* if detached, returns 1 on success. if not, it returns the pointer to the
 * promise. in both cases NULL is returned if fails. */
static inline promise_t* __submit_task(thread_queue_t* ctx,
    ...
    return !detached ? promise : (promise_t*)1;
...
int submit_task_detached(thread_queue_t* ctx, void* (*callback)(void*),
                         void* arg) {
    if (!__submit_task(ctx, callback, arg, 1)) return -1;
...
promise_t* submit_task(thread_queue_t* ctx, void* (*callback)(void*),
                       void* arg) {
    return __submit_task(ctx, callback, arg, 0);
}
...
ssize_t remove_tasks(thread_queue_t* ctx, int (*pred)(const void*)) {
    ...
    pthread_mutex_lock(&ctx->queue_mtx);
    /* life could be a dream if i could have the callback expect task_t* and
     * cast it to a function that expects void* and it would just work, but no,
     * the language doesn't allow that. i must require the user to manually cast
     * void* to task_t* in their callbacks, even though it will always be
     * task_t* in this case, which is sucky design. day ruined. */
    ssize_t retval = queue_remove_if(&ctx->queue, pred);
    pthread_mutex_unlock(&ctx->queue_mtx);
    return retval;
}

static inline void* __consume_promise(promise_t* promise) {
    sem_destroy(&promise->done);
